...
export class SessionService {
  ...
  get isAuthenticated(): boolean {
    if (!this._client.oidc) {
      return this._client.user.isAuthenticated
    }
  ...
  async initialize(): Promise<void> {
    if (!this._client.oidc || !isBrowser()) return
  ...
  async getAccessToken(): Promise<string | null> {
    if (!this._client.oidc || !isBrowser()) return null
  ...
  checkExpiration(): void {
    if (!this._client.oidc || !this._user) return
  ...
  private getManager(): UserManager {
    if (!this._client.oidc) {
      throw new Error(
        'OIDC session management requires the client to be constructed with `oidc` options.'
      )
    }
    if (!isBrowser()) {
  ...
  private getCurrentUrl(): string {
    if (!isBrowser()) {
      return this._client.oidc
        ? this._client.resolveAppUrl(this._client.oidc.redirectPath)
        : this._client.resolveAppUrl('/')
    }
