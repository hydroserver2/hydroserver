import uuid

from ninja import Router, Path, Query
from django.db import transaction

from interfaces.auth.security import session_auth, oidc_auth, apikey_auth, basic_auth, anonymous_auth
from interfaces.api.http.request import HydroServerHttpRequest
from interfaces.api.services.sta import SampledMediumAPIService
from interfaces.api.schemas import PaginatedResponse, ItemResponse, CreatedResponse
from interfaces.api.schemas.sta.vocabulary import (
    VocabularyQueryParameters,
    VocabularyItemQueryParameters,
)
from interfaces.api.schemas.sta.sampled_medium import (
    SampledMediumResponse,
    SampledMediumPostBody,
    SampledMediumPatchBody,
)

sampled_medium_router = Router(tags=["Sampled Mediums"])
sampled_medium_service = SampledMediumAPIService()


@sampled_medium_router.get(
    "",
    auth=[session_auth, oidc_auth, apikey_auth, basic_auth, anonymous_auth],
    response={
        200: PaginatedResponse[SampledMediumResponse],
        400: str,
        401: str,
    },
    by_alias=True,
)
def get_sampled_mediums(
    request: HydroServerHttpRequest,
    query: Query[VocabularyQueryParameters],
):
    """
    Get Sampled Mediums.
    """

    return 200, sampled_medium_service.list(
        principal=request.principal,
        offset=query.offset,
        limit=query.limit,
        sortby=query.sortby,
        filtering=query.dict(exclude_unset=True),
    )


@sampled_medium_router.post(
    "",
    auth=[session_auth, oidc_auth, apikey_auth, basic_auth],
    response={
        201: CreatedResponse,
        401: str,
        403: str,
    },
    by_alias=True,
)
@transaction.atomic
def create_sampled_medium(
    request: HydroServerHttpRequest,
    data: SampledMediumPostBody,
):
    """
    Create a new Sampled Medium.
    """

    return 201, sampled_medium_service.create(
        principal=request.principal,
        data=data,
    )


@sampled_medium_router.get(
    "/{sampled_medium_id}",
    auth=[session_auth, oidc_auth, apikey_auth, basic_auth, anonymous_auth],
    response={
        200: ItemResponse[SampledMediumResponse],
        400: str,
        401: str,
        403: str,
    },
    by_alias=True,
    exclude_unset=True,
)
def get_sampled_medium(
    request: HydroServerHttpRequest,
    sampled_medium_id: Path[uuid.UUID],
    query: Query[VocabularyItemQueryParameters],
):
    """
    Get a Sampled Medium.
    """

    return 200, sampled_medium_service.get(
        principal=request.principal,
        uid=sampled_medium_id,
    )


@sampled_medium_router.patch(
    "/{sampled_medium_id}",
    auth=[session_auth, oidc_auth, apikey_auth, basic_auth],
    response={
        204: None,
        401: str,
        403: str,
    },
    by_alias=True,
)
@transaction.atomic
def update_sampled_medium(
    request: HydroServerHttpRequest,
    sampled_medium_id: Path[uuid.UUID],
    data: SampledMediumPatchBody,
):
    """
    Update a Sampled Medium.
    """

    sampled_medium_service.update(
        principal=request.principal,
        uid=sampled_medium_id,
        data=data,
    )
    return 204, None


@sampled_medium_router.delete(
    "/{sampled_medium_id}",
    auth=[session_auth, oidc_auth, apikey_auth, basic_auth],
    response={
        204: None,
        401: str,
        403: str,
    },
    by_alias=True,
)
@transaction.atomic
def delete_sampled_medium(
    request: HydroServerHttpRequest, sampled_medium_id: Path[uuid.UUID]
):
    """
    Delete a Sampled Medium.
    """

    return 204, sampled_medium_service.delete(
        principal=request.principal, uid=sampled_medium_id
    )
