...
def _transformations_url(task_id):
    return f"{DATA_PRODUCT_TASKS_URL}/{task_id}/transformations/aggregation"
...
# --- get_aggregation_transformations --------------------------------------------------
...
# --- create_aggregation_transformation --------------------------------------------------
...
def test_create_aggregation_transformation_succeeds_for_workspace_owner(client):
    ...
    assert response.json()["aggregationMethod"] == "mean"
...
def test_create_aggregation_transformation_returns_400_when_timezone_missing_for_iana(client):
    ...
    response = client.post(
        _transformations_url(task.id),
        data=_transformation_body(output_ds, input_ds, timezoneType="iana"),
        content_type="application/json",
    )
...
# --- update_aggregation_transformation --------------------------------------------------------
...
def test_update_aggregation_transformation_succeeds_for_workspace_owner(client):
    ...
    response = client.patch(
        ...
        data={"aggregationMethod": "sum"},
        content_type="application/json",
    )

    assert response.status_code == 200
    assert response.json()["aggregationMethod"] == "sum"
...
# --- delete_aggregation_transformation --------------------------------------------------------
