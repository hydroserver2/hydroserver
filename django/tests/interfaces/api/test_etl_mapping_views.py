...
MAPPING_FIELDS = {"id", "sourceIdentifier", "targetDatastreamId"}


def _mappings_url(task_id):
    return f"{ETL_TASKS_URL}/{task_id}/mappings"


def _detail_url(task_id, mapping_id):
    return f"{_mappings_url(task_id)}/{mapping_id}"
...
def test_get_etl_mappings_includes_mapping_for_workspace_owner(client):
    ...
    response = client.get(_mappings_url(task.id))
...
def test_get_etl_mappings_returns_404_for_outsider(client):
...
def test_create_etl_mapping_succeeds_for_workspace_owner(client):
    ...
    response = client.post(
        _mappings_url(task.id),
        data={"sourceIdentifier": "col_a", "targetDatastreamId": str(target.id)},
    ...
    detail = client.get(_detail_url(task.id, mapping_id))
    assert detail.json()["data"]["sourceIdentifier"] == "col_a"
    assert detail.json()["data"]["targetDatastreamId"] == str(target.id)


def test_create_etl_mapping_returns_403_without_edit_permission(client):
    workspace = WorkspaceFactory()
    task = _make_task(workspace)
    monitoring_site = MonitoringSiteFactory(workspace=workspace)
    target = DatastreamFactory(monitoring_site=monitoring_site)
...
def test_get_etl_mapping_returns_200_for_workspace_owner(client):
    ...
    response = client.get(_detail_url(task.id, mapping.id))
...
def test_get_etl_mapping_include_sideloads_target_datastream(client):
    ...
    response = client.get(
        _detail_url(task.id, mapping.id), {"include": "targetDatastream"}
    )
...
def test_get_etl_mappings_include_does_not_scale_queries_with_mapping_count(client):
    ...
    with CaptureQueriesContext(connection) as small:
        client.get(_mappings_url(task.id), {"include": "targetDatastream"})
...
def test_update_etl_mapping_succeeds_for_workspace_owner(client):
    ...
    response = client.patch(
        _detail_url(task.id, mapping.id),
    ...
    detail = client.get(_detail_url(task.id, mapping.id))
...
def test_delete_etl_mapping_succeeds_for_workspace_owner(client):
    ...
    response = client.delete(_detail_url(task.id, mapping.id))

    assert response.status_code == 204
    assert client.get(_detail_url(task.id, mapping.id)).status_code == 404
