...
| O | elevationM | The elevation of the Location in meters. | Decimal |
...
| O | organizationId | A one-to-one identifier for the Organization with which the User is affiliated. Each User may be affiliated with at most one Organization, and each Organization may be affiliated with at most one User. | Integer |
...
| O | periodicTaskId | A one-to-one identifier for the associated scheduler record in `django_celery_beat`. Each Task maps to at most one periodic task record. | Integer |
